        ...
        for n in range(1):
            ...
            if my_file.is_file():
                ...
                for line in data:
                    ...
                    penLine = QPen(QColor(Qt.red))
                    self.painter.setBrush(QColыor(Qt.white))
                    
                    Size = self.GetStationPopularity('', self.pytrends)

                    self.painter.drawEllipse(float(x)-RADIUS/100*Size, float(y)-RADIUS/100*Size, (RADIUS/100*Size)*2, (RADIUS/100*Size)*2)
