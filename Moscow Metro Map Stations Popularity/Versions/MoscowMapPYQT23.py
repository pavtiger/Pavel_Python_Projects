...
class Example(QWidget):
    ...
    def keyPressEvent(self, event):
        if event.key() == 16777216:#Esc
            ...
            stationsName = self.NameReading(FILE_STATIONS_NAME)
        ...
        for n in range(1):
            ...
            if my_file.is_file():
                ...
                for line in data:
                    ...
                    self.painter.drawEllipse(float(x)-RADIUS/100*self.stationSize[i], float(y)-RADIUS/100*self.stationSize[i], (RADIUS/100*self.stationSize[i])*2, (RADIUS/100*self.stationSize[i])*2)
                    i+=1
