...
class Example(QWidget):
    ...
    def FileDrawing(self, fileName: str):
        ...
        for n in range(9):
            with open(fileName + str(n + 1) + '.txt', 'r') as f:
                data = f.read().split('\n')
                f.close()
            ...
            for line in data:
                ...
                if lastX is not None or lastY is not None:
                    ...
                    self.painter.drawLine(lastX, lastY, float(x), float(y))
    ...
    def initUI(self):
        ...
        self.overlay = QImage()
        self.overlay.load('Moscow Metro Map Stations Popularity\\Nothing.bmp')
